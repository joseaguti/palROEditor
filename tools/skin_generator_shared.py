...
for gender in [sprite_f_sf,sprite_m_sf]:
    for i in tqdm.tqdm(range(HEAD_COLOR)):
        name = hair_name+f"1_"+gender+f"_{i}.pal"
        ...
        for x in range(N_SKINS):
            ...
            for ii in range(16):
                for jj in range(16):
                    ...
                    new_buffer += c
            name = hair_name + f"{1}_" + gender + f"_{i+x*HEAD_COLOR}.pal"
            path = os.path.join(path_hair, name)
            path = path.replace("\\\\", "\\")
            ...
            outname_file = f"{out_name}_{gender}_{i+x*HEAD_COLOR}.pal"
        ...
        for i in range(BODY_COLOR):
           ...
           # print(path,len(buffer))
            ...
            for x in range(N_SKINS):
                ...
                path_f = os.path.join(OUTPATH, path)
                os.makedirs(os.path.dirname(path_f), exist_ok=True)
                outname_file = f"{out_name}_{gender}_{i+x*BODY_COLOR}.pal"
                path_f = os.path.join(os.path.dirname(path_f), outname_file)
                with open(path_f, "wb") as fp:
                    fp.write(new_buffer)
                #print("-->",path,len(new_buffer))